...
class OscilloscopeApp(tk.Tk):
    def __init__(self, port, baudrate=115200):
        ...
        self.trigger_edge = 'rising'  # Default trigger to rising edge
    ...
    def create_widgets(self):
        ...
        self.trigger_button = ttk.Button(
            control_frame, text="Trigger: Rising", command=self.toggle_trigger
        )
        self.trigger_button.pack(side=tk.LEFT, padx=5, pady=5)
    ...
    def toggle_trigger(self):
        # Toggle between rising and falling edge for triggering
        self.trigger_edge = 'falling' if self.trigger_edge == 'rising' else 'rising'
        self.trigger_button.config(
            text=f"Trigger: {self.trigger_edge.capitalize()}"
        )
            ...
            if not self.is_paused:
                try:
                    ...
                    if not self.triggered:
                        if (self.trigger_edge == 'rising' and value > self.trigger_level and self.last_value <= self.trigger_level) or (
                            self.trigger_edge == 'falling' and value < self.trigger_level and self.last_value >= self.trigger_level):
                            self.triggered = True
